...
export default function Login() {
    ...
    //     navigate('/dashboard/');
    // } else {
    //     navigate('/dashboard/users'); // or '/' if you prefer
    // }
    if (request.status === 200) {
    const user = response.user;
    localStorage.setItem("user", JSON.stringify(user));
    const userRole = response.user.role; // assumes backend returns user.role
            ...
            <section className="bg-light">
                <div className="container d-flex flex-column align-items-center justify-content-center min-vh-100 py-4">
                    <a href="#" className="mb-4 text-dark text-decoration-none d-flex align-items-center">
                        ...
                        <Link to={'/'}> <span className="h4 mb-0 fw-bold">ThinkPen🖋</span></Link>
                    </a>
